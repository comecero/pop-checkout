...
gulp.task("popup-client", function () {
    return gulp.src(["./app/client/*.js"])
      .pipe(gulp.dest("./dist/js/"));
});

gulp.task('dist', function (done) {
    sequence('concat-pages-js', 'popup-client', 'compress', 'sourcemap', 'sass', function () {

        // Read the version number
        var version = fs.readFileSync("./version.html", "utf8");

        // Add headers with the release number to each of the distribution files.
        gulp.src(['./dist/js/pages.js', './dist/js/pages.min.js', './dist/js/popup.js', './dist/js/popup.min.js']).pipe(header("/*\nComecero Popup Cart version: " + version + "\nhttps://comecero.com\nhttps://github.com/comecero/cart\nCopyright Comecero and other contributors. Released under MIT license. See LICENSE for details.\n*/\n\n")).pipe(gulp.dest('./dist/js/'));
        done();
    });
});
